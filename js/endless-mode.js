...
var EndlessMode = {
  ...
  /**
   * Награда за уровень — 12 🪙
   */
  getReward: function(level) {
    return 12;  // ← БЫЛО 15, СТАЛО 12
  },

  /**
   * Бонус за каждые 5 уровней — +15 🪙 фиксированный
   */
  getBonus: function(level) {
    if (level % 5 !== 0) return 0;
    return 15;  // ← БЫЛО 25, СТАЛО 15
  },
  ...
  /**
   * Сохранить финальный результат (при проигрыше)
   */
  saveResult: function(level) {
    try {
      var raw = localStorage.getItem('flow_endless');
      var data = raw ? JSON.parse(raw) : { best: 0, totalLevels: 0 };
      data.totalLevels = (data.totalLevels || 0) + level;
      data.best = Math.max(data.best || 0, level);
      localStorage.setItem('flow_endless', JSON.stringify(data));
      return data;
    } catch (e) {}
    return { best: level, totalLevels: level };
  },
