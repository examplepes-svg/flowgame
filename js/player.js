...
/* === PATCH п.5: хелпер замены эмодзи-валют на HTML-иконки === */
function _replaceCurrencyEmoji(str) {
  if (typeof str !== 'string') return str;
  return str
    .replace(/💎/g, '<span class="gem-icon"></span>')
    .replace(/🪙/g, '<span class="coin-icon"></span>');
}
...
function giveGems(amount) {
  ...
  if (window.UI) {
    var msg = t('toast.gems_added', '💎 +{n} алмазов!', { n: amount });
    msg = _replaceCurrencyEmoji(msg);
    UI.showToast(msg, 2000);
  }
}
...
function buyItem(itemType, key, preferredCurrency) {
  ...
  if (window.UI) {
    UI.confirm(
      ...
      [
        { label: priceCoins + ' <span class="coin-icon"></span>', class: 'yes', action: function() {
            doBuyItem(state, scene, item, itemType, key, ownedList, 'coins', priceCoins);
        }},
        { label: priceGems + ' <span class="gem-icon"></span>', class: 'ad', action: function() {
...
function doBuyItem(state, scene, item, itemType, key, ownedList, currency, price) {
  ...
  if (currency === 'coins') {
    ...
    spentCoins = price;
    if (window.UI) {
      var msg = t('toast.bought_coins', 'Куплено за {n} 🪙', { n: price });
      msg = _replaceCurrencyEmoji(msg);
      UI.showToast(msg);
    }
  } else {
    state.gems -= price;
    if (window.UI) {
      var msg = t('toast.bought_gems', 'Куплено за {n} 💎', { n: price });
      msg = _replaceCurrencyEmoji(msg);
      UI.showToast(msg);
    }
  }
