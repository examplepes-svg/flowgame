...
var THEMES = {
  ...
  anime: {
    ...
    colors: [
      0xffffff, // белый
      0xffe4ec, // бледно-розовый
      0xffd6e8, // светло-розовый
      0xe8d5f5, // бледно-сиреневый
      0xd6e4ff, // бледно-голубой
      0xd5f5e3, // бледно-мятный
      0xfff0d6, // бледно-персиковый
      0xf5e6d3  // бледно-бежевый
    ]
  }
};
...
var BACKGROUNDS = {
  ...
  /* === ЭКСКЛЮЗИВ FALLPASS — фон «Осенний лес» === */
  anime_forest: {
...
/* ============================================================
   ЕЖЕДНЕВНЫЕ ЗАДАНИЯ
   Храним в localStorage только { id, progress, claimed, completed }.
   Контент (desc / target / reward) берётся из шаблона + t() при КАЖДОМ рендере,
   поэтому перевод всегда актуален.
   ============================================================ */
...
var DailyTasks = {
  templates: {
    ...
    medium: [
      ...
      { id: 'fast3',        get desc() { return t('task.fast3', 'Пройди 3 уровня за минуту'); },             target: 3,   reward: 45 },
      ...
      { id: 'fast5',        get desc() { return t('task.fast5', 'Пройди 5 уровней за 3 минуты'); },          target: 5,   reward: 45 },
      { id: 'noHint5',      get desc() { return t('task.noHint5', 'Пройди 5 уровней без подсказок'); },      target: 5,   reward: 45 }
    ],
    hard: [
      { id: 'play10',       get desc() { return t('task.play10', 'Пройди 10 уровней'); },                     target: 10,  reward: 60 },
      { id: 'fast5',        get desc() { return t('task.fast5', 'Пройди 5 уровней за 2 минуты'); },          target: 5,   reward: 60 },
      { id: 'perfect5',     get desc() { return t('task.perfect5', 'Пройди 5 уровней без ошибок'); },        target: 5,   reward: 60 },
  ...
  generate: function() {
    ...
    if (saved) {
      try {
        ...
        // Проверяем структуру: у слотов должны быть валидные id из шаблонов
        if (data.date === today &&
  ...
  /**
   * Возвращает задания с УЖЕ ПЕРЕВЕДЁННЫМ desc / target / reward.
   * Использовать для рендера — перевод будет актуален на текущем языке.
   */
  getForRender: function() {
    ...
    ['easy', 'medium', 'hard'].forEach(function(diff) {
      ...
      out[diff] = {
        id: slot.id,
        desc: tpl.desc,           // геттер вызовется СЕЙЧАС → актуальный язык
        target: tpl.target,
        reward: tpl.reward,
...
/* ============================================================
   ...
   Та же логика: храним только id + progress, контент берём из шаблона.
   ============================================================ */
var WeeklyTasks = {
  templates: {
    easy: [
...
/* Задания FallPass — desc — это КЛЮЧ локализации, переводим при рендере */
var BP_TASKS = [
  ...
  { id: 'bp_fast_3', desc: 'task.bp_fast_3', fallbackDesc: 'Пройди 3 уровня за минуту', target: 3, type: 'fast_levels' },
  { id: 'bp_fast_5', desc: 'task.bp_fast_5', fallbackDesc: 'Пройди 5 уровней за 2 минуты', target: 5, type: 'fast_levels' },
  { id: 'bp_fast_7', desc: 'task.bp_fast_7', fallbackDesc: 'Пройди 7 уровней за 3 минуты', target: 7, type: 'fast_levels' },
...
/* --- BP_REWARDS: цикл 10 уровней (бесплатно) --- */
var BP_REWARDS = [
...
/* --- BP_PREMIUM_REWARDS: цикл 10 уровней (премиум) --- */
...
/* ============================================================
   ЗАДАНИЯ FALLPASS
   В localStorage храним только { id, progress, completed, claimed }.
   Контент (desc / target / type) берём из BP_TASKS по id при КАЖДОМ рендере.
   ============================================================ */
...
var BPTasks = {
  ...
  generate: function() {
    ...
    // Генерим заново: только id + служебные поля
  ...
  /**
   * Возвращает список задач С ПЕРЕВЕДЁННЫМ desc — для рендера.
   * Перевод всегда актуален на текущем языке.
   */
  getForRender: function() {
    var data = this.generate();
    var self = this;
    ...
    for (var i = 0; i < data.tasks.length; i++) {
      ...
      result.push({
        id: slot.id,
        desc: t(tpl.desc, tpl.fallbackDesc),   // ← перевод СЕЙЧАС
        target: tpl.target,
        type: tpl.type,
        progress: slot.progress,
        completed: slot.completed,
...
/* ============================================================
   FALLPASS — МАГАЗИН И ОБМЕН ЛИСТИКОВ
   ============================================================ */

/* Список товаров магазина BP (кроме обмена) */
var BP_SHOP_ITEMS = [
  { id: 'bg_anime_forest', type: 'background', key: 'anime_forest', leafPrice: 60, get name() { return t('bg.anime_forest', 'Осенний лес'); },     preview: 'assets/ui/battlepass_bg.webp' },
  { id: 'theme_anime',     type: 'theme',      key: 'anime',        leafPrice: 60, get name() { return t('theme.anime', 'Тема «Аниме»'); },          preview: null },
  { id: 'trail_anime',     type: 'trail',      key: 'anime',        leafPrice: 60, get name() { return t('trail.anime', 'След «Аниме»'); },          preview: '#ffb7d5' }
];

/* Обмен листиков на монеты/алмазы */
var LEAF_EXCHANGE = {
...
/* ============================================================
   FALLPASS — ЕДИНАЯ ФУНКЦИЯ ДОБАВЛЕНИЯ BP XP
   ============================================================ */
