...
var DuelScene = new Phaser.Class({
  ...
  init: function(data) {
    // Явно приводим к boolean — чтобы никакие данные из прошлых сессий
    // не могли сохраниться, даже если Phaser переиспользует сцену.
    this.isTraining = !!(data && data.training);
  ...
  create: function() {
    ...
    this.events.on('shutdown', function() {
      ...
      // Восстанавливаем стандартную полосу прогресса
      var progressWrap = document.querySelector('.progress-bar-wrap');
      if (progressWrap) progressWrap.style.display = '';
      // Восстанавливаем надпись "Уровень N"
      var uiLevelRestore = document.getElementById('ui-level');
      if (uiLevelRestore) uiLevelRestore.style.display = '';
  ...
  setupHTMLUI: function() {
    ...
    // Скрываем надпись "УРОВЕНЬ N" / "ДУЭЛЬ" / "ТРЕНИРОВКА" — в дуэли не нужна.
    // UI.setLevel всё равно выставляет текст "Уровень N", поэтому просто прячем.
    var uiLevel = document.getElementById('ui-level');
    if (uiLevel) uiLevel.style.display = 'none';
    ...
    // Скрываем стандартную полосу прогресса (в дуэли она не нужна)
    var progressWrap = document.querySelector('.progress-bar-wrap');
  ...
  showDuelResult: function(winner) {
    ...
    safeBind('duel-again', function() {
      ...
      UI.hideAll();
      // 400 мс — чтобы Phaser успел корректно завершить shutdown сцены
      setTimeout(function() { DuelSearch.start(false); }, 400);
    });
    ...
    safeBind('duel-to-menu', function() {
      ...
      self.scene.stop('Duel');
      UI.hideAll();
      setTimeout(function() { DuelMenu.show(); }, 400);
    });
