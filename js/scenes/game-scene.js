...
var GameScene = new Phaser.Class({
  ...
  create: function() {
    ...
    // === PATCH п.6: не рестартим сцену при resize, а пересчитываем layout ===
  ...
  onVictory: function() {
    ...
    if (!alreadyCompleted && this.mode === 'normal') {
      ...
      if (state.bpStats) {
        ...
        // === PATCH п.3: BPTasks.updateProgress('fast_levels', 1) удалён
        // (задача bp_fast_* больше не существует в BP_TASKS).
        // state.bpStats.fast_levels оставлен для совместимости.
        if (window.BPTasks) {
          BPTasks.updateProgress('play_levels', 1);
          BPTasks.updateProgress('earn_coins', reward);
